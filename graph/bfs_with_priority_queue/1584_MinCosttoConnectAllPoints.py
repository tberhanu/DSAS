...
class Solution:
    def minCostConnectPoints(self, points: List[List[int]]) -> int:
        ...
        5. when all points visited, done !

        Time: 
            O(N**2): graph construction, where N = len(points)
            N**2 points push/pop >> O(N**2 log(N**2)) = O(N**2 logN)
            total: O(N**2 logN)
        Space: 
            O(N**2): for the adjacency list representation of the graph
            O(N**2): for minHeap
            O(N): for visited set
            total: O(N**2)
        """
